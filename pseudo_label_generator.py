...
def main():
    ...
    print("Just checking: ", unlabeled_data_path, face_detector_path, output_path) # debug
    # Load Unlabeled Data 
    unlabeled_data_list = glob.glob(os.path.join(unlabeled_data_path, '*'))  ## original
    # unlabeled_data_list = glob.glob(os.path.join(unlabeled_data_path, '**', '*'), recursive=True) ## my update
    ...
    cluster_label = kmeans.fit_predict(vaf_list) ## original
    ## MY CODE:
    if len(vaf_list) == 0:
        raise ValueError("vaf_list is empty. Ensure that data is being extracted correctly.")
    # cluster_label = kmeans.fit_predict(np.array(vaf_list).reshape(-1, 1))  # Reshape to 2D    ##updated
    ...
    for image, label in data:
        ...
        video_name = image.split("-")[0] ## changed by me
    ...
    ## MY CODE
    output_file = 'video_pseudo_labels.json'
    with open(os.path.join(output_path, output_file), 'w') as f:
        json.dump(pseudo_video, f, indent=4)
    ## MY CODE end here
    ...
    for filename in os.listdir(unlabeled_data_path):
        ...
        video_name = filename.split('-')[0] ## changed by me
        label = None
    ...
    output_file = 'image_pseudo_labels2.json'
    with open(os.path.join(output_path, output_file), 'w') as f:
...
if __name__ == '__main__':
    main()
>>>>>>> 428ff4759a3b22e7d03eabe26bfbee40fc8e4e76
